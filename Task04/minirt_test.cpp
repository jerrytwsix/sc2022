...
{
public:
    ThreadsPool(int _threads_num, const Scene &scene, const Image &image, ViewPlane &viewPlane): viewPlane(viewPlane)
    {
        this->_threads_num = _threads_num;
        this->scene = scene;
        this->image = image;
        startThreads(_threads_num);
    };

    void startThreads(int _threads_num)
    {
        for (int i = 0; i < _threads_num; i++)
            _threads_vec.emplace_back([this]() 
            {
                 threadFunction(); 
            });
    }
    ...
    void threadFunction()
    {
        auto point = _tasks_queue.pop();
        while (point.x >= 0)
        {
            const auto color = viewPlane.computePixel(scene, point.x, point.y, 1);
            image.set(point.x, point.y, color);
            point = _tasks_queue.pop();
        }
    }

    void pushTask(Point point)
    {
        _tasks_queue.push(point);
    }

    void join()
    {
        for (auto &_thread: _threads_vec)
            _tasks_queue.push({-1, -1}); 
...
private:
    int _threads_num;
    LockingQueue<Point> _tasks_queue;
    vector<thread> _threads_vec;
    Scene scene;
    Image image;
    ViewPlane viewPlane;
};
...
int main(int argc, char **argv) {
    ...
    auto pool = new ThreadPool(4, scene, image, viewPlane)
